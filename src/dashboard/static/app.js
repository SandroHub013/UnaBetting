...
const ACTS = {
  ...
  chat: { title: 'Chat agente', render: (body) => {
    body.appendChild(el('div', 'sb-note',
      `Modello locale <b>qwen3.5:9b</b> via Ollama (zero costi API). ` +
      `Ha accesso a: match di oggi, segnali, metriche modello, bankroll, scan live.`));
    body.appendChild(el('div', 'sb-section', 'Prova a chiedere'));
    ['quali sono i match di oggi?', 'ultimi segnali del modello?',
     'com’è messo il modello?', 'stato del bankroll?',
     'fai uno scan live e dimmi cosa trovi'].forEach(q => {
      const b = el('button', 'sb-item', '✦ ' + q);
      b.onclick = () => { openChat(); const inp = $('#chat-input'); if (inp) { inp.value = q; inp.focus(); } };
      body.appendChild(b);
    });
    body.appendChild(el('div', 'sb-note', 'Nota: la prima risposta a freddo carica il modello in VRAM (~1-2 min); poi resta caldo.'));
    openChat();
  }},
  ...
  docs: { title: 'Documentazione', render: async (body) => {
    ...
    ['README.md', 'EXPERIMENTS.md', 'ALPHA_FINDINGS.md', 'PROJECT_EVALUATION.md'].forEach(p => {
  ...
  openTab('panel:chat', '✦ Chat', (pane) => {
    pane.innerHTML = `<div class="chat-host">
      <div class="chat-msgs" id="chat-msgs">
        <div class="chat-msg bot">Ciao! Sono l'agente di Mission Control (qwen3.5:9b, locale).
Chiedimi dei match di oggi, dei segnali, del modello o del bankroll — uso i dati veri dell'app.</div>
      </div>
