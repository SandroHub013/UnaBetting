...
ax.plot(fpr, tpr, color='#2980b9', linewidth=2.5, label='XGBoost (AUC = 0.884)')
...
n_bets = 564
edges = rng.normal(0.06, 0.12, n_bets)
stakes = np.abs(rng.normal(25, 15, n_bets))
stakes = np.clip(stakes, 5, 100)
outcomes = np.where(rng.random(n_bets) < 0.773, 1, -1)
